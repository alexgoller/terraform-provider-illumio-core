---
page_title: "Provider: Illumio Segmentation"
sidebar_current: "docs-illumio-core-index"
description: |-
  The Illumio Segmentation provider is used to interact with and manage resources in the Illumio Policy Compute Engine (PCE).
---

# Illumio Segmentation Provider
...
- `write_href_file` (Boolean, Deprecated) Whether to record provisionable object HREFs to hrefs.csv for the external `provision` binary. Set to false when using the `illumio-core_provisioning` resource, otherwise the file accumulates entries that are never consumed. Default value: true
