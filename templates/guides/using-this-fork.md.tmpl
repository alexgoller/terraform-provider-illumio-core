...
~> **Version numbering.** This fork releases as `2.0.2`, which sorts **above**
upstream's 1.x line, and it is served under the same `illumio/illumio-core`
source address. Two consequences:
>
> - **Always pin the version exactly** (`version = "2.0.2"`, not `>= 2.0` or
>   `~> 2.0`).
...
`2.0.2` makes it obvious which provider is in use:

```bash
# Linux / macOS. Adjust OS_ARCH to match your machine, e.g. darwin_arm64,
# darwin_amd64, linux_amd64.
OS_ARCH="$(go env GOOS)_$(go env GOARCH)"
DEST=~/.terraform.d/plugins/registry.terraform.io/illumio/illumio-core/2.0.2/${OS_ARCH}
...
```
- Installing illumio/illumio-core v2.0.2...
- Installed illumio/illumio-core v2.0.2 (unauthenticated)
```
...
- **`terraform providers lock` does not work against a scoped mirror.** It
  fails with `the previously-selected version 2.0.2 is no longer available`,
...
- **Pick a version that upstream will never publish** (`2.0.2` here). Terraform
